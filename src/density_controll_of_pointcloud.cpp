...
vector<CloudAPtr> CloudPtrList;
vector<Eigen::Vector2f> tmpTimeList;
vector<float> score;
	...
	for(size_t i=0;i<list_size;i++){
		...
		*tmp_1pc_after_downsampling = *CloudPtrList.at(i);
		...
		for(size_t j=0;j<pc_after_size;j++){
			if(border_score > tmp_1pc_after_downsampling->points[i].intensity){
				
				cout << "け" << endl;
				tmp_1pc_after_downsampling->points.erase(tmp_1pc_after_downsampling->points.begin()+j);	
			}
		}
